...
def arg_parse():
    parser = argparse.ArgumentParser(description="Run Qwen3.5-VL Agent with local-kernel code interpreter")
...
def make_output_basename(args):
    name = f"{args.dataset_name}_id_2_qwen35_tool_inference"
    if args.use_zoom_in:
        name += "_zoom_in"
    if args.use_code_interpreter:
...
if __name__ == "__main__":
    ...
    print("-" * 40)
    print(f"Using model: {args.qwen35_model_name}")
    print("-" * 40)
    main(args)