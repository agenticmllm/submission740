"""
OpenAI互換API + 手動ReActエージェントループによるデータセットバッチ推論スクリプト。
...
使用例:
  python run_openai_agent_batch.py \
...
def strip_base64_from_messages(messages: List[dict]) -> List[dict]:
    """
    pickle保存時のサイズ肥大化を防ぐため、messages内のbase64画像データを
    プレースホルダーに置換したコピーを返す。元のリストは変更しない。
    """
    ...
    for msg in stripped:
        ...
        if isinstance(content, list):
            for part in content:
                ...
                # Responses API style input_image
...
def run_agent_loop(
    ...
    Returns:
        list[dict]: 会話履歴リスト。各要素は {"role": ..., "content": ..., ...} 形式。
                    base64画像データは保存サイズ削減のため除去済み。
    """
    ...
    # ツールが無い場合は tool_choice / tools を送らない
    ...
    for iteration in range(max_iterations):
        ...
        # 思考テキストの蓄積
        if msg.content:
            final_text = msg.content  # 最後のテキストが最終回答になる
            _dbg(f"  💭 [Reasoning]\n{msg.content.strip()}\n")
            ...
            try:
                if func_name == "zoom_in":
                    ...
                    if code_interpreter_session is None:
                        ...
                        # 画像が生成された場合はマルチパートで返す (zoom_inと同じ方式)
                        if ci_result["images"]:
                            tool_content = [
...
def run_agent_loop_responses(
    ...
    """
    Responses API を使う手動 tool loop。
    GPT-5 系のように chat.completions を受けないモデル向け。
    """
...
def parse_args():
    ...
    parser.add_argument("--dataset_name", type=str, default="holisafe",
                        help="Dataset name: holisafe, mm_safety_bench, vsl_bench, mssbench, mmmu")
    ...
    #parser.add_argument("--parallel_tool_calls", type=str, default="false",
    #                    choices=["true", "false"],
    #                    help="Enable parallel tool calls (default: true)")
...
def main():
    ...
    # Override the API key if using Azure OpenAI
    ...
    # Model name mapping (placeholder identifiers; configure to your inference endpoint).
    args.model_short = args.model  # For file naming
    ...
    # parallel_tool_callsを文字列からboolに変換
    #parallel_tool_calls_bool = args.parallel_tool_calls == "true"

    # コードインタプリタセッション (使う場合のみ初期化)
    ...
    #print(f"Parallel tool calls: {parallel_tool_calls_bool}")
    if args.debug:
        print(f"🐛 DEBUG MODE: 10 samples, verbose output, no saving")
    print("-" * 60)

    # ========================================
    # デバッグモード: 10サンプル、全出力、保存なし
    # ========================================
    if args.debug:
        ...
        for i, entry in enumerate(debug_entries):
            ...
            try:
                ...
                # 最終回答 = 最後のassistantメッセージのcontent
                final_msg = ""
    ...
    for i, entry in enumerate(tqdm(entries_to_process, desc="Processing")):
        ...
        try:
            ...
            #if "truncated" in err_msg or "cannot identify" in err_msg or "could not process image" in err_msg:
